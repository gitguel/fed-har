"""
Supervised baseline: train ResNetSE1D_5 from scratch (random init, all params free).

Runs across the same data regimes and seeds as eval_lfr.py for a fair comparison:
  5 regimes × 4 seeds = 20 runs.

Best model (lowest val_loss) is loaded for testing.

Outputs
-------
  results/supervised_results.csv   — acc, f1, recall, precision per run
  logs/supervised/<run_name>/      — full training curves (CSVLogger)
"""
...
def make_model(lr: float = 1e-3) -> SimpleSupervisedModel:
...
for regime_label, n_spc in regimes:
    for seed in SEEDS:
        run_idx += 1
        run_name = f"supervised_{regime_label}spc_seed{seed}"
...
results_path = RESULTS_DIR / "supervised_results.csv"
